...
func (c *TConfig) refineProjects() {
	...
	for i, _p := range config.Projects {
		_project := TProject{
			Name:         "",
			Healthchecks: map[string]checks.THealthcheck{},
		...
		for j, _h := range _p.Healthchecks {
			_healthcheck := checks.THealthcheck{
				Name:       "",
				Checks:     map[string]checks.TCheckConfig{},
			...
			_healthcheck.Checks = make(map[string]checks.TCheckConfig)
...
func getMergeOptions() *conjungo.Options {
	...
	opts.SetTypeMergeFunc(
		reflect.TypeOf(checks.TCheckParameters{}),
		func(t, s reflect.Value, o *conjungo.Options) (reflect.Value, error) {
			tFoo := t.Interface().(checks.TCheckParameters)
			sFoo := s.Interface().(checks.TCheckParameters)
...
func hostOrUrl(c checks.TCheckConfig) string {
	if c.Url != "" {
		return c.Url
	}
	return c.Host
