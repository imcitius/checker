...
After successful pipeline finish, you should get your app deployed at `https://fly.io/apps/<checker-name>`.
Let's try to ping it by http:
```
❯ curl https://<checker-name>.fly.dev/healthcheck
Ok!
```

With default configuration it will just check https://google.com availability once per minute and output to log.
App's log can be monitored using: `flyctl logs -a <checker-name>`.
...
`URL to monitor` shoud be set to Checker's healthcheck url we used before (`https://<checker-name>.fly.dev/healthcheck`).
